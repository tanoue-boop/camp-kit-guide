...
- **総記事数: 99記事**（`ls content/posts/*.mdx | wc -l` で確認）
...
> 直近の増加分: 2026-06-08 のバッチで 90→94（施策3: 焚き火シート/火ばさみ/防寒グローブ/大容量ポータブル電源）→ 99（施策4: 防水リュック/二次燃焼焚き火台/折りたたみソーラーパネル/ダッチオーブン/封筒型寝袋）。詳細は `docs/seo-change-log.md` を参照。
