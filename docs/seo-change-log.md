...
6/23の差別化リライト3本（#1 backpack-capacity / #2 tent-size / #3 dutch-oven）に続き、グループB（おすすめ◯選型）の勝ち筋＝「判断軸を競合より体系的に表で整理＋買った後の実用情報＋初心者向け結論の明示（E-E-A-T偽装なし）」を残りのテント記事へ展開した。
...
- **#5 stylish-camp-tent（グループB・本コミット）**: KW「おしゃれ キャンプ テント」。デザインタイプ別比較表（ワンポール／ベル型／カマボコ／パップ／ロッジ／ドームの7列×6行）・素材傾向表（ポリ／TC＝ポリコットン）・シーン別早見表を新設し、感覚的な「映え」を判断軸として体系化。ProductCard5商品・比較表1・FAQ5問はKEEP。updatedAtを6/29に更新（dateModified反映）。

**効果測定（次回7/7）**
- グループBの型（判断軸の体系化＋実用情報）でテント系記事の順位が動くかを確認。動いた型をグループB残（nanga-sleeping-bag / camp-chair-highback / two-room-tent-guide 他）へ横展開する。
...
- 動いた型を、同グループの残り記事へ高速展開する。グループA残：sleeping-bag-temp-guide 等。グループB残：nanga-sleeping-bag / camp-chair-highback / two-room-tent-guide 他（solo-tent-overall・stylish-camp-tent は実施済み＝6/29セクション参照）。
