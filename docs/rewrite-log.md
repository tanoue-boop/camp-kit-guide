...
- 着手可能（`着手可否=○`）かつ仮説が `要判断` 以外の記事は **camp-table-set／car-camp-lighting／bluetti-power／torch-burner／camp-table-folding** の5本（3サイクル目で portable-fridge／camp-gear-sale-timing を消化）。次サイクルはこの中から選ぶ
- `2026-10-18まで待機` は karrimor-backpack／gregory-backpack／deuter-backpack（09-21 に mysteryranch-backpack への内部リンク1行＋updatedAt を変更）と portable-fridge／camp-gear-sale-timing（09-21 に3サイクル目で施策済）の5本。待機明けの候補は gregory-backpack（容量別 H2）と deuter-backpack（普段使い H2＋リンク元追加）
- 3サイクル目（09-21）でリンク元として本文1行＋updatedAt を変更した Tier1 台帳外の記事: **camp-cooler-box-overall／portable-power-vehicle-camp／cooler-ice-pack／snowpeak-tent** の4本。いずれも 2026-10-18 まで待機扱い（Tier2 以下を台帳化する際に、この4本は 09-21 変更ありとして扱う）
