...
| camp-table-set | 30 | 282 | 8 | 2.84% | 11.9 | 2026-08-21〜2026-09-17（28日） | 内部リンク不足 | 着手可否=○（09-04 以降未更新）。主要クエリ5種（テーブルセットの表記ゆれ）がいずれも順位14前後で page2 に揃っており、CTR でなく順位の問題。09-04 に人数別・単品との違いの H2 を追加済みで構造施策は実施済み。自サイトからの内部リンクは3本（captain-stag-table／solo-camp-cot／takibi-table）で、同じテーブル系で camp-table-folding にはリンクしている dod-table／low-style-table／outdoor-kitchen-table／hanging-rack／water-jug からは0本。これら5本から「椅子付きセット」文脈で1行ずつ送る | | 施策日＋28日 | |
...
| portable-fridge | 8 | 210 | 1 | 0.48% | 7.7 | 2026-08-21〜2026-09-17（28日） | CTR | 着手可否=○（09-20 の変更は楽天リンク形式のみ・本文/title 不変）。順位7.7 で CTR 0.48%（90日でも 2.35%）とサイト平均の1/8。クエリ判明は8件（表示の1割）で、判明分の「車載冷蔵庫」系は順位3.1。title は「ポータブル冷蔵庫」のみで「車載冷蔵庫」表記が無い（本文には11回・第5位の商品名にもある）。title/description に「車載冷蔵庫」を併記して CTR を狙う。あわせて自サイトからの内部リンクが0本（11位以下では torch-burner／camp-gear-sale-timing と並び最少）なので、保冷・電源系の記事から送る | | 施策日＋28日 | |
...
| camp-gear-sale-timing | 4 | 156 | 7 | 4.49% | 6.2 | 2026-08-21〜2026-09-17（28日） | 内部リンク不足 | 着手可否=○（08-20 公開以降未更新）。順位6.2・CTR 4.49% と良好だが、自サイトからの内部リンクが0本。情報記事のため商品5選記事の「価格・買い時」文脈（テント・クーラーボックス・ポータブル電源の記事）から送りやすい。判明クエリ（テント／クーラーボックスの安い時期）に対応する記事からのリンクを優先 | | 施策日＋28日 | |
...
| camp-table-folding | 2 | 70 | 1 | 1.43% | 9.2 | 2026-08-21〜2026-09-17（28日） | CTR | 着手可否=○（06-29 以降未更新）。順位9.2 で CTR 1.43%（90日 0.91%）とサイト平均の1/3以下が90日通して続く。判明クエリ2件（比較・サイズ意図）で原因は特定できないが、title は主要KWを含んでおり、H2 が「この記事でわかること」「各製品の詳細レビュー」「比較表まとめ」と旧テンプレ構成（2024-06 公開・最古）。内部リンクは8本と十分。title/description の見直しと構成の標準化が候補 | | 施策日＋28日 | |
...
- 着手可能（`着手可否=○`）かつ仮説が `要判断` 以外の記事は **camp-table-set／portable-fridge／car-camp-lighting／bluetti-power／camp-gear-sale-timing／torch-burner／camp-table-folding** の7本。次サイクルはこの中から選ぶ
- `2026-10-18まで待機` は karrimor-backpack／gregory-backpack／deuter-backpack の3本（いずれも 09-21 に mysteryranch-backpack への内部リンク1行＋updatedAt を変更）。待機明けの候補は gregory-backpack（容量別 H2）と deuter-backpack（普段使い H2＋リンク元追加）
- 商品構成そのものが問題と判断できた記事は 11位以下には無く、`_file/article-fix-backlog.tsv` への追加登録は無し
- 内部リンク0本の記事: portable-fridge／camp-gear-sale-timing／torch-burner の3本
