...
- **2026-09-21 追加（campkit-20260921-25）**: `scripts/check-card-name-vs-sku.cjs`（ProductCard の name/price と楽天実SKUの整合検出）の結果から一括起票した。追加語彙は次の2つ。
  ...
  - `issue_type=asin_mismatch`（2026-09-22 追加・campkit-20260921-36 §B・キュー#16）: 既設置の `amazonAsin` が別変種（サイズ違い・Plus モデル等＝verdict `model_mismatch`）や別商品（本体のカードにグランドシートの ASIN 等＝`different_product`）を指している案件。対処は **`amazonAsin` の差し替えのみ**（name／price／本文は触らない）。detail に Amazon 側の変種一覧から拾った候補 ASIN を書くが**未照合**なので、差し替え前に dp ページで変種名・価格を確認する。検出は `scripts/check-amazon-asin.cjs`（`--static` は静的検査のみでネットに出ない／`--verify N` で Amazon dp を N 件だけ照合・間隔 2 秒・429/503/CAPTCHA で exit 2／`--judge slug#rank#id=verdict --note …` で人手判定を記録）。出力 `_file/amazon-asin-check.tsv`（全 1121 カード・`link_form`＝amazonAsin／amazonUrl／legacy_source_amazon／none）。第1弾（20 枚照合）では **air-frame-tent が 5 枚中 4 枚誤り**（Cowork の Amazon 実データ選定時に変種 ASIN を取り違えた記事）だったので、同経路（`fce3941` 系「Amazon実データ5選」）の記事を優先して照合する。
    ...
    - **2026-09-22 追加（campkit-20260921-54・キュー#16 第6弾の運用メモ）**: **消滅 ASIN に対する Amazon の正規 404 ページ（「ページが見つかりません」・kailey-kitty）は HTML コメントの定型文「自動アクセス」に `parseDp.captcha` が反応し、`--verify` が `blocked_by_amazon`（`amazon_stock=http=404 captcha`）で exit 2 停止する**（本物のブロックは 503＋Robot Check。直前後の dp が正常取得できていればブロックではない）。対処は検出器を変えず、`--judge <key>=404 --note …` で verdict を `404` に訂正してから **残り枚数で `--verify N` を再開**する（`--only` で選び直さない＝対象集合は変わらない）。`404` の行は `asin_mismatch`（priority A・購入導線が死んでいる）で起票し、対処は「再探索 1 回 → 無ければ `amazonAsin` 除去」。第6弾の誤りは 6 枚＝色・サイズ違い 5（camp-pillow#2 TAN／camp-rainwear#2 M ドレスネイビー／#3 M ベージュ／camp-sleeping-mat#1 ベージュ／#2 チャコールブラウン。いずれも変種一覧に候補あり）＋セット買い ASIN 1（camp-skillet#1 UG-3028＋竹製プレート→単品 B06XVZGX2D はセットページ内の構成品リンク `dp_pba_bcd` から拾える）。
    - **2026-09-22 追加（campkit-20260921-57・キュー#29 第3弾）**: 上記 54 の偽陽性は**検出器側で解消済み**。`parseDp` は `notFound` を先に決め、`captcha` は `notFound` でないときだけ立てる。`verify()` も HTTP 404 を `dp.captcha` より先に扱うので、**正規 404 ページは停止せず `verdict=404` のまま処理が続く**（429/503・Robot Check の即停止は不変）。54 の `--judge …=404` で訂正してから `--verify N` を再開する運用はもう要らない。あわせて `PURE_DIGIT_MODEL_MIN` を 7→6 にした（ユニフレーム 683040・コールマン 205588・イスカ 117212 等の 6 桁カタログ番号を拾う。全 1121 枚で測定し `static_flags` 分布は完全に不変・増えたトークン 25 個は全て実在の型番）。⚠️ この時点では `check-card-name-vs-sku.cjs` の同名定数は 7 のままで両者が食い違っていたが、**campkit-20260921-60 §A で 6 に揃えて解消済み**（下記）。
    - **2026-09-22 追加（campkit-20260921-60・キュー#29 第4弾）**: (1) `check-card-name-vs-sku.cjs` の `PURE_DIGIT_MODEL_MIN` も 7→6 にし、**2 つの検出器の食い違いは解消**（全 1121 カードで測定＝`modelTokens` が変わるのは 24 枚・増えるトークン 25 個は全て実在の型番・消えるトークン 0。`flags` が変わるのは 1 行だけで `isuka-sleeping-bag#2`＝カード name の品番 111803 に対し実 SKU のメーカー型番は 111809 → `model_mismatch` が新たに立つ＝真の不一致の顕在化。`sku_selected` の変化 0 行）。⚠️ 両者の `modelTokens` は**この定数以外のロジックも元から違う**（`check-card-name-vs-sku.cjs` は 4 文字以上・英字必須・数字2個以上・半角化・販促語除外・前方一致トークンの間引きあり／`check-amazon-asin.cjs` は 3 文字以上・数字1個以上で大文字化のみ）。揃えたのは純数字の下限だけ。 (2) **`price_gap` は「購入ボックスの無いページ」では空にする**＝`amazon_stock` の base（`; ` より前）が `no_buybox` で始まる行。`parseDp` は `id="corePrice"` が取れないとページ内の任意の `class="a-price-whole"` にフォールバックするため、その `amazon_price` は「買える価格」ではない（実例: `coleman-lantern#3` の `-68%`）。空にするのは導出列の `price_gap` だけで **`amazon_title`／`amazon_price`／`amazon_stock`／`verdict`／`seller_type`／`note` は監査証跡として残す**。キュー#19（`price_gap ≧ +15%` かつ `seller_type` が official/amazon）の列挙が買えないページ由来の数字で汚れるのを防ぐのが目的。 (3) **恒久ルール: 変更禁止リスト（`FROZEN_SLUGS`）の記事に対する起票は `status=blocked` で起票する**（`pending` は「いま着手してよい」の意味）。2026-10-19 以降に `pending` へ昇格させる。
