...
- **Cowork Amazonリンク化フローの継続展開**: `_file/amazon-link-worksheet.tsv` の残り約530商品を、同じ三者分業（Coworkログアウト検索 → まーくんが `amzn.to` 発行 → Claude Code が ProductCard に `amazonUrl` 設置）でバッチ展開する。流入のある記事から優先。フローは19記事82商品で実証済み・精度良好。同一商品の型番違いは楽天商品ID／バッジで区別する。
- **6/23 SEOレポート定期測定**: GASメニュー「📊 SEOレポート」を実行し、6/8比較で効果測定する（詳細は `docs/seo-change-log.md` の定期実施スケジュールを参照）。
