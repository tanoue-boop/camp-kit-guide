...
CONTENT_DIR = r'C:\Users\tanou\camp-kit-guide\content\posts'
...
def build_block(rank, id_, description, badge, new_data):
    lines = ['<ProductCardMdx',
             f'  rank="{rank}"',
             f'  id="{id_}"',
             f'  name="{new_data["name"]}"',
             f'  description="{description}"',
             f'  price="{new_data["price"]}"',
             f'  rakutenRating="{new_data["rakutenRating"]}"',
             f'  rakutenReviewCount="{new_data["rakutenReviewCount"]}"',
             f'  affiliateUrl="{new_data["affiliateUrl"]}"',
             f'  source="rakuten"',
             ]
    if badge:
        lines.append(f'  badge="{badge}"')
    lines.append(f'  image="{new_data["image"]}"')
    lines.append('/>')
    return '\n'.join(lines)


def process_file(stem, reps):
    filepath = os.path.join(CONTENT_DIR, stem + '.mdx')
    ...
    pattern = r'<ProductCardMdx\n(?:  \S.*\n)*?/>'
    blocks = re.findall(pattern, content)

    if len(blocks) != 5:
        print(f'WARNING: {stem} has {len(blocks)} blocks (expected 5)')

    for block in blocks:
        rank_str = get_attr(block, 'rank')
        try:
            rank_idx = int(rank_str) - 1
        except ValueError:
            print(f'WARNING: bad rank "{rank_str}" in {stem}')
            continue
        if rank_idx < 0 or rank_idx >= len(reps):
            print(f'WARNING: rank {rank_str} out of range in {stem}')
            continue

        id_ = get_attr(block, 'id')
        description = get_attr(block, 'description')
        badge = get_attr(block, 'badge')

        new_block = build_block(rank_str, id_, description, badge, reps[rank_idx])
        content = content.replace(block, new_block, 1)

    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)

    print(f'OK: {stem}.mdx ({len(blocks)} blocks replaced)')


for stem, reps in REPLACEMENTS.items():
    process_file(stem, reps)
